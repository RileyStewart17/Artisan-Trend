...
if getattr(sys, 'frozen', False):
    ...
    import win32api
    basedir = win32api.GetLongPathName(sys._MEIPASS)
else:
...
try: 
    ...
    dir_name = config['Directory']['Location']
    ...
    dir_name = askdirectory(parent=root,initialdir="/",title='Please select a directory')+'/'
    root.destroy()
    if dir_name != '/':
        config = configparser.RawConfigParser()
        config.optionxform = str 
        ...
        config.set('Directory', 'Location', dir_name)
...
while not loaded:
    try:
        ...
        onlyfilesAC = [f for f in listdir(dir_name) if isfile(join(dir_name, f))] # List of files present in directory
...
for i in range(len(onlyfilesAC)):
    AClist.append(open_file(dir_name + onlyfilesAC[i]))
...
# Changes global variable 'dir_name' to something else if changed
def change_directory_variable(updated_variable):
    global dir_name
    dir_name = updated_variable
...
def change_directory():
    ...
    root.withdraw()
    messagebox.showinfo('Air Canada Trends', 'Please select the folder that contains the excel report files.')
    dir_name2 = askdirectory(parent=root,initialdir="/",title='Please select a directory')+'/'
    root.destroy()
    if dir_name2 != '/':
        config.read('settings.ini')
        config.set('Directory', 'Location', dir_name2)
        with open('settings.ini', 'w') as configfile:
            config.write(configfile)
    change_directory_variable(dir_name2)
...
def plotter(system, test):
    ...
    if system == 'CL':
        ...
        for t in range(len(AClist)):
            ...
            if test.lower() == 'clcond':
                name = 'Closed Loop Conductivity'
                for i in range(NCT):
                    if len(datadict) == 0:
                        datadict['data{0}'.format(i)] = []
                    try:
            ...
            if test.lower() == 'clmo':
                name = 'Closed Loop Molybdenum'
                for i in range(NCT):
                    if len(datadict) == 0:
                        datadict['data{0}'.format(i)] = []
                    try:
                        d1 = file_data[3][i]
                        ind = np.where((variables == 'Mo') | (variables == 'Molybdenum'))[0][0]
                        u1 = str(d1[ind])
                        if u1 in ban:
                            u1 = np.nan
                        elif u1[0] in ban2:
...
class app():
    def __init__(self):
        ...
        self.window.iconbitmap(basedir+'/artisan_logo.ico')       
        self.create_widgets()
        self.window.grid()
    ...
    def create_widgets(self):
        ...
        button18 = ttk.Button(entry_frame,
                                 text="Nitrite",
        ...
        button13 = ttk.Button(entry_frame,
                                 text="Conductivity",
                                 command=lambda: plotter('CL','clcond'))
        button19 = ttk.Button(entry_frame,
                                 text="Molybdenum",
                                 command=lambda: plotter('CL','clmo'))
        ...
        button18.grid(row=1, column=0, columnspan=1, sticky=tk.E + tk.W + tk.N + tk.S)
        button19.grid(row=0, column=5, columnspan=1, sticky=tk.E + tk.W + tk.N + tk.S)
...
class listbox():
    ...
    def CreateWidgets(self):
        ...
        self.btnRemove.grid(column=20, row=3, stick=tk.E, pady=5)
        self.btnAdd = tk.Button(entry_frame2, text="+", command = lambda: add_item(self.samplealtnames))
        self.btnAdd.grid(column=10, row=3, stick=tk.E, pady=5)
